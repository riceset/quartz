...
In Insert Mode you can type on the document as in any other text editor. In Normal Mode you can execute commands to navigate through the file or modify it.
...
`A` (Shift-a) will put you on insert mode on the end of the line.
`O` (Shift-o) will put on insert mode on the previous line.
...
- `f` followed by a word `a` moves the cursor to the next occurrence of the word `a` on a line.
- `t` followed by a word `a` moves your cursor a word before the next occurrence of the word `a` word on a line.
...
- `dgg` Delete from the current line until the beginning of the document.
